...
String getPaymentStatusText(String? status, String? method) {
  ...
  } else if (status == BOOKING_STATUS_PENDING_APPROVAL &&
    ...
    File? croppedImage = await Navigator.push<File?>(
      ...
      MaterialPageRoute(
        builder: (context) => CropImageScreen(
            key: ValueKey(imageFile.path), imageFile: imageFile),
      ),
    );
...
List<LanguageDataModel> filterDefaultLanguages(List<String> defaultLanguages) {
  final filteredList = languageList()
      .where((lang) => defaultLanguages.contains(lang.languageCode))
      .toList();
