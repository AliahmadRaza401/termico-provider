...
Future<List<ServiceData>> getSearchList(
  ...
  try {
    ...
    req =
        '?$categoryIds$providerIds$subCategorys$serviceType$approvalStatus$searchPara$pages$perPages';
...
Future<SelectZoneModelResponse> selectedZones({int? providerId}) async {
  return SelectZoneModelResponse.fromJson(await handleResponse(
      await buildHttpResponse('provider-zones?provider_id=$providerId',
          method: HttpMethodType.GET)));
}
...
Future<List<ZoneResponse>> getZoneWithPagination({
  ...
  try {
    ...
    SelectZoneModelResponse res = SelectZoneModelResponse.fromJson(
        await handleResponse(await buildHttpResponse(
            'provider-zones?provider_id=$providerId&per_page=$newPerPage&page=$page',
            method: HttpMethodType.GET)));
...
Future<List<PaymentSetting>> getPaymentGateways(
  ...
  try {
    // External gateways are intentionally disabled in this build.
    return [];
  } catch (e) {
    throw e;
  }
...
Future<List<PostJobData>> getPostJobList(
  ...
  try {
    ...
    log('🔍 Fetching jobs from API: $url');
...
Future<BaseResponseModel> buyOfferPackage({
  ...
  MultipartRequest multiPartRequest =
      await getMultiPartRequest('buy-offer-package');
...
Future<OfferPackageStatusResponse> getOfferPackageStatus() async {
  try {
    log('═══════════════════════════════════════════════════════════════');
    log('📡 API CALL: get-offer-package-status');
    log('═══════════════════════════════════════════════════════════════');
    log('🔄 Method: GET');
    log('📍 Endpoint: get-offer-package-status');
    log('⏰ Request Time: ${DateTime.now().toIso8601String()}');
    log('───────────────────────────────────────────────────────────────');

    var response = await buildHttpResponse('get-offer-package-status',
        method: HttpMethodType.GET);
    var responseData = await handleResponse(response);

    log('───────────────────────────────────────────────────────────────');
    log('✅ API RESPONSE RECEIVED: get-offer-package-status');
    log('📦 Raw Response Data:');
    log(jsonEncode(responseData));
    log('───────────────────────────────────────────────────────────────');

    OfferPackageStatusResponse res =
        OfferPackageStatusResponse.fromJson(responseData);

    if (res.data != null) {
      log('📊 Parsed Offer Package Status:');
      log('   • has_package: ${res.data!.hasPackage}');
      log('   • has_active_package: ${res.data!.hasActivePackage}');
      log('   • package_id: ${res.data!.packageId}');
      log('   • package_name: ${res.data!.packageName}');
      log('   • package_price: ${res.data!.packagePrice}');
      log('   • offers_per_month: ${res.data!.offersPerMonth}');
      log('   • offers_used: ${res.data!.offersUsed}');
      log('   • offers_remaining: ${res.data!.offersRemaining}');
      log('   • status: ${res.data!.status}');
      log('   • start_at: ${res.data!.startAt}');
      log('   • end_at: ${res.data!.endAt}');
      log('   • can_send_offer: ${res.data!.canSendOffer}');
      log('   • payment_id: ${res.data!.paymentId}');
      log('   • payment_method_name: ${res.data!.paymentMethodName}');
      log('   • canSendOffers (computed): ${res.data!.canSendOffers}');
      log('   • cannotSendOfferReason: ${res.data!.cannotSendOfferReason}');
    } else {
    ...
    log('───────────────────────────────────────────────────────────────');
    log('❌ API ERROR: get-offer-package-status');
    log('🚫 Error Details: $e');
