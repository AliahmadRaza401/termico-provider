...
class _PricingPlanScreenState extends State<PricingPlanScreen> {
  ...
  bool get isIosExternalPaymentBlocked =>
      isIOS && !appConfigurationStore.isInAppPurchaseEnable;

  Future<void> saveSubscriptionPurchase(
      {required String paymentType, String transactionId = ''}) async {
      ...
      return AppScaffold(
        ...
        body: Center(
          child: Padding(
            ...
            child: Text(
              'Subscription purchases are hidden on iOS in this build.',
              ...
              return AnimatedScrollView(
                ...
                children: [
                  ...
                  Text(languages.selectPlanSubTitle,
                          style: secondaryTextStyle())
                      .center(),
                  if (isIosExternalPaymentBlocked) ...[
                    16.height,
                    Container(
                      ...
                      child: Text(
                        'iOS purchases are available only through Apple In-App Purchase. Configure the iOS in-app product to activate paid plans on Apple devices.',
                        style: secondaryTextStyle(),
                      ),
                    ),
                  ],
                ...
                onTap: () async {
                  if (selectedPricingPlan!.identifier == FREE) {
                    await saveSubscriptionPurchase(
                        paymentType: PAYMENT_METHOD_COD, transactionId: '');
                  } else {
                    if (isIOS) {
                      if (!appConfigurationStore.isInAppPurchaseEnable) {
                        toast(
                            'Paid plans on iPhone and iPad must use Apple In-App Purchase.');
                        return;
                      }
