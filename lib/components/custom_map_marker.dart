...
class CustomMapMarker {
  ...
  static Future<BitmapDescriptor> createServiceImageMarker({
    ...
    try {
      ...
      if (serviceImage != null) {
        ...
        // Fallback if image fails to load - draw colored circle with service icon
        print('⚠️ Using fallback icon for marker (image not available)');
        ...
        // Draw handyman/service icon instead of shopping bag
        final TextPainter iconPainter = TextPainter(
          textDirection: TextDirection.ltr,
        );
        iconPainter.text = TextSpan(
          text: String.fromCharCode(Icons.handyman.codePoint),
          style: TextStyle(
            fontSize: imageSize * 0.45,
            fontFamily: Icons.handyman.fontFamily,
            color: Colors.white,
          ),
        );
        iconPainter.layout();
  ...
  /// Loads an image from network URL with caching
  static Future<ui.Image?> _loadNetworkImage(String url) async {
    try {
      ...
      final ui.Codec codec = await ui.instantiateImageCodec(
        bytes,
        targetWidth: 200, // Higher quality
        targetHeight: 200,
      );
      ...
      print('❌ Error loading network image from $url: $e');
      return null;
    }
  }
